<x-layout>
    ...
    <div class="bg-white min-w-[80%] mt-[20px] rounded-[5px] grid grid-cols-[auto_1fr_auto] p-4 gap-2 overflow-x-scroll">
        ...
        <span>Customer Service 2 <br>{{ $user->nama }}</span>
        <span></span>
                        ...
                        <td>
                            <div class="p-2 place-items-center flex justify-center gap-2">
                                @if ($d->status == 'diproses')
                                    <form action="{{ route('kelola.panel.kemas', $d->id) }}" method="POST">
                                        @csrf
                                        <button onclick="return confirm('anda yakin barang sudah dikemas?')"
                                            class="border p-2 px-3 bg-[#29f165] rounded-[5px] text-[#1e4f30]">pengemasan</button>
                                    </form>
                                @elseif($d->status == 'dikemas')
                                    <form action="{{ route('kelola.panel.kirim', $d->id) }}" method="POST">
                                        @csrf
                                        <button onclick="return confirm('anda yakin barang siap dikirim?')"
                                            class="border p-2 px-3 bg-[#29f165] rounded-[5px] text-[#1e4f30]">pengiriman</button>
                                    </form>
                                    {{-- @else --}}
...
<script>
    ...
    window.onload = function() {
        ...
        @elseif (session('proses'))
            Swal.fire({
                icon: 'success',
                title: 'berhasil mengemas',
                text: "melanjutkan proses pengemasan",
                showCancelButton: false,
                showConfirmButton: false,
                timer: 1800,
                timerProgressBar: true,
        ...
        listen(transaksi, '{{ route('kelola.panel.proses') }}', function(data) {
